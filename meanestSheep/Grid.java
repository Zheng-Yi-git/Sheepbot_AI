...
public class Grid {
	...
	public static void generatePolicy(
			Node[][] grid,
			AngrySheep sheep,
			SheepdogBot bot) {
		...
		// policy states the state and returns the best possible next move for the bot
		// depending on the current state (sheep_location, bot_location)
		HashMap<Key, Node> policy = new HashMap<Key, Node>();
		double beta = 0.98;

		int count = 1;
		// Iterate through all states and determine which node to go to is the best for
		// bot
		for (Key state : states.keySet()) {
			...
			bot.row = b.x;
			bot.col = b.y;
			// There is program end when the two following conditions are met, so there is
			// not possible next Node for these cases
			if (sheep.row == bot.row && sheep.col == bot.col) {
				continue;
			} else if ((sheep.row == 15 && sheep.col == 15) &&
					!(bot.row == 15 && bot.col == 15)) {
				continue;
			}
			...
			// for all the neighbors of the bot iterate through every possible location in
			// which the sheep may go and update the T*(State) to minimum possible value
			// And at the end of iteration set the policy for the state to be the bot
			// neighbor which yielded the min value
			for (Node bot_neighbor : bot.neighbors) {
				...
				val = reward + (beta * sum);
				// if val is lesser than the finalmi set val to the finalmin and update the x,y
				// values to the state of bot_neighbor
				if (val < finalmin) {
					finalmin = val;
					x = bot_neighbor.row;
					y = bot_neighbor.col;
	...
	public static void valueIteration(
		...
		// int i = 0;
		// if the max difference between prev and curr falls below 0.01 we can say that
		// the values have converged and we can exit the algorithm
		while (!convergence) {
			int count = 1;
			double max = -100;
			// for all the states in previous iterate through the valueiteration algorithm
			// to find the min value of each state
			for (Key keys : prev.keySet()) {
				...
				// if the sheep and the bot occupy the same location initialize state to
				// infinity else if goal state initialize to 0
				if ((sheep.row == bot.row && sheep.col == bot.col)) {
				...
				// for every bot_neighbor in combination with the average of every possible
				// sheep location use value-iteration algorithm to get minimum T*(State)
				for (Node bot_neighbor : bot.neighbors) {
					double reward = -1;
					double val = 0;
					double sum = 0;
				...
				// On each iteration get the curr val and prev val, calculate the absolute
				// difference between the two and get the max difference, if the max is less
				// than smallval (0.01) set convergence to true
			...
			method1(curr);
			// set curr to prev and if convergence is false begin again else end of
			// valueiteration
			prev.putAll(curr);
		}
	...
	// If the bot is view of the sheep we modify the neighbors for the
	// valueiteration and generatepolicy algorithm to give more informed results
	public static void modSheepNeighbors(
			Node[][] grid,
			AngrySheep sheep,
			SheepdogBot bot) {
	...
	// Grabs the initial estimated T* Values based on a heuristic that uses the
	// Manhattan Distance from the bot to the sheep and then from the sheep to the
	// goal plus 2 to take into account the barrier of the sheep pen
			...
			for (int j = 0; j < 31; j++) {
				for (int m = 0; m < 31; m++) {
					for (int n = 0; n < 31; n++) {
						...
						// goal states
						if ((p1.x == 15 && p1.y == 15) && !(p2.x == 15 && p2.y == 15)) {
							...
							double estimate = Math.abs(p2.y - p1.y) +
									Math.abs(p2.x - p1.x) +
									Math.abs(p1.x - 15) +
									Math.abs(p1.y - 15) +
									2;
	...
	// Generate random sheep position, if initial position is blocked Node then
	...
	// Generate random sheepdog position, if initial position is blocked Node then
	// generate another position
	...
	private static void generateSheepView(Node[][] grid, AngrySheep sheep) {
		...
		// set vrow and vcol to the top corner of the 5x5 view and work our way down to
		// vrowend, vcolend
		while (vrow < vrowend) {
			while (vcol < vcolend) {
				// if dimensions are out of bounds skip
				if (vrow < 0 || vcol < 0 || vrow > 30 || vcol > 30) {
					vcol++;
					continue;
				}
	...
	// get neighbors of the sheep, including self because the sheep can also decide
	// to stay put
	...
	// get neighbors of the bot, including self because the bot can also decide to
	// stay put
	public static void getBotNeighbors(Node[][] grid, SheepdogBot bot) {
		...
		if (row + 1 <= 30 && !grid[row + 1][col].blocked) {
			bot.neighbors.add(grid[row + 1][col]);
		}
		if (col - 1 >= 0 && !grid[row][col - 1].blocked) {
			bot.neighbors.add(grid[row][col - 1]);
		}
		if (col + 1 <= 30 && !grid[row][col + 1].blocked) {
			bot.neighbors.add(grid[row][col + 1]);
		}
		if (row - 1 >= 0 && col - 1 >= 0 && !grid[row - 1][col - 1].blocked) {
			bot.neighbors.add(grid[row - 1][col - 1]);
		}
		if (row - 1 >= 0 && col + 1 <= 30 && !grid[row - 1][col + 1].blocked) {
			bot.neighbors.add(grid[row - 1][col + 1]);
		}
		if (row + 1 <= 30 && col + 1 <= 30 && !grid[row + 1][col + 1].blocked) {
			bot.neighbors.add(grid[row + 1][col + 1]);
		}
		if (row + 1 <= 30 && col - 1 >= 0 && !grid[row + 1][col - 1].blocked) {
			bot.neighbors.add(grid[row + 1][col - 1]);
		}
	}

	// Algorithm that determines how the sheep moves
	private static void sheepMove(
			Node[][] grid,
			AngrySheep sheep,
			SheepdogBot bot) {
		Node move;
	...
	// loadinitialestimates loads the data from the T*(State) into a
	// hashmap<Key,Double> which is returned to either valueIteration or
	// generatePolicy
	...
	// Loads the Policy from generatedPolicy to be used in TrapSheep to guide the
